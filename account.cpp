...
// Account::Account() {}
Account::Account(Info* uf, Username* us, privLevel l = basic) : _info(uf->clone()), _user(us->clone()), _privilege(l) {}
Account::Account(const Account& acc) : /* _info(acc._info), _user(acc._user), _privilege(acc._privilege)*/ _info(acc._info->clone()), _user(acc._user->clone()), _history(acc._history), _privilege(acc._privilege) {}
