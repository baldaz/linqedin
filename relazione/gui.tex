...
di \textit{QGridLayout} contenuti all'interno di un \textit{Widget} padre \lq\lq globale \rq\rq mediante un \texit{QStackedLayout} che richiamando lo
...
\begin{enumerate}
    ...
    formato da \textit{QPushButton}, possiede inoltre l'overload di alcuni eventi del mouse, come ad esempio il \texit{dragging} o l'ingrandimento
    ...
    che deriva pubblicamente \textit{QTextBrowser} costituita da un campo edit in modalità \textit{readonly} e due campi dati \texit{QString} utili a
    ...
    che le connessioni sono visualizzate sempre attraverso l'oggetto \textit{Gui\_DispInfo}, inoltre vengono generati alcuni pulsanti contenuti in una \texit{QToolBar}
...
\end{enumerate}
Ad ogni modifica effettuata viene sempre richiamato il metodo di salvataggio dall'interfaccia \texit{LinqClient}, in ogni caso eventuali modifiche vengono rese permanenti al logout dell'applicazione.
