...
protected:
    ...
    private:
        ...
        struct lessThan {
            const User* c;
            lessThan(const User* cl) : c(cl){}
            inline bool operator()(const SmartPtr<User>& s, const SmartPtr<User>& s2) const {
                return (c->similarity(s) <= c->similarity(s2));
            }
        };
    public:
        ...
        void sortedRes(vector<SmartPtr<User> >&) const;
    };
    class linkedWith {
    private:
        int _offset;
