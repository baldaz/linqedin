...
void User::searchFunctor::operator()(const SmartPtr<User>& spu) {
    ...
    switch(_s_type) {
        ...
        case 3:
            if(!_wanted.empty() && _wanted.at(0) == ':') {
            ...
            else if(!_wanted.empty() && _wanted.at(0) == '>') {
                if(uf) {
                    vector<string> skills = uf->skills();
                    vector<string>::iterator itr = skills.begin();
                    for(; itr < skills.end(); ++itr)
                ...
                if(uf) {
                    vector<SmartPtr<User> > candidates;
                    vector<string> skills = uf->skills();
                    ...
                    if((utilities::Utils::toLowerCase(uf->name()) == _wanted ||
                        ...
                        candidates.push_back(spu);
                    }
                    sortedRes(candidates);
                    for(vector<SmartPtr<User> >::iterator itr = candidates.begin(); itr != candidates.end(); ++itr) {
                        std::cout << (*itr)->account()->username().login() << std::endl;
                        if(_result.size() < 400) {
                            _result.insert(std::pair<string, string>((*itr)->account()->username().login(), (*itr)->showInfo() + "\n" + (*itr)->net()->printHtml()));
                            (*itr)->addVisit();
                            if((*itr)->account()->prLevel() == executive) {
                                ExecutiveUser* eu = dynamic_cast<ExecutiveUser*> (&(*(*itr)));
...
void User::searchFunctor::sortedRes(vector<SmartPtr<User> >& cd) const {
    std::sort(cd.begin(), cd.end(), lessThan(_caller));
}
